...
async function run() {
  ...
  try {
    ...
    const snapshot = { metadata: { timestamp: new Date().toISOString(), version: '2.0', source: 'africom-backup' }, stats, data };
    ...
    const newFile = await drive.files.create({
      ...
      media: { mimeType: 'application/json', body: fileBuffer },
    });
    ...
    const deletions = oldFiles.data.files.filter(f => f.id !== newFile.data.id);
